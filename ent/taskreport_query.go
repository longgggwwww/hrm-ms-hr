...
func (trq *TaskReportQuery) QueryReporter() *EmployeeQuery {
	query := (&EmployeeClient{config: trq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := trq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (trq *TaskReportQuery) Clone() *TaskReportQuery {
	...
	return &TaskReportQuery{
		...
		withReporter: trq.withReporter.Clone(),
...
func (trq *TaskReportQuery) WithReporter(opts ...func(*EmployeeQuery)) *TaskReportQuery {
	query := (&EmployeeClient{config: trq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	trq.withReporter = query
	return trq
}
...
func (trq *TaskReportQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*TaskReport, error) {
	...
	if query := trq.withReporter; query != nil {
		if err := trq.loadReporter(ctx, query, nodes, nil,
			func(n *TaskReport, e *Employee) { n.Edges.Reporter = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (trq *TaskReportQuery) loadReporter(ctx context.Context, query *EmployeeQuery, nodes []*TaskReport, init func(*TaskReport), assign func(*TaskReport, *Employee)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*TaskReport)
	for i := range nodes {
...
func (trq *TaskReportQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := trq.ctx.Fields; len(fields) > 0 {
		...
		if trq.withTask != nil {
			_spec.Node.AddColumnOnce(taskreport.FieldTaskID)
		}
		if trq.withReporter != nil {
			_spec.Node.AddColumnOnce(taskreport.FieldReporterID)
		}
	}
